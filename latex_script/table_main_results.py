...
def main():
    ...
    output_dir = os.path.join(root, "parsing/output_parsing")
    with open(os.path.join(output_dir, "NDCG.json"), "r") as f:
        NDCG_output_dict_parsing = json.load(f)
    ...
    output_dir = os.path.join(root, "parsing/output_parsing_dataset")
    with open(os.path.join(output_dir, "NDCG.json"), "r") as f:
        NDCG_output_dict_parsing_dataset = json.load(f)
    ...
    output_dir = os.path.join(root, "parsing/output_parsing_uriel")
    with open(os.path.join(output_dir, "NDCG.json"), "r") as f:
        NDCG_output_dict_parsing_uriel = json.load(f)


    latex_head = "\\begin{table*}[t!]\n\\begin{center}\n\\begin{tabular}{c|cccc}\n\\midrule\n\\bf Model & \\bf MT & \\bf EL & \\bf POS & \\bf Parsing \\\\\n\\midrule\n"

    latex_filling_dataset = ""

    feature_name = "Overlap word-level"
    latex_filling_dataset += "%s & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f \\\\\n" % \
        (symbol_dict[feature_name],
         ...
         NDCG_output_dict_parsing[feature_name]["avg"] * 100,
         NDCG_output_dict_parsing[feature_name]["std"] * 100)

    feature_name = "Overlap subword-level"
    latex_filling_dataset += "%s & %.1f $\\pm$ %.1f & --- & --- & --- \\\\\n" % \
        (symbol_dict[feature_name],
         NDCG_output_dict_mt[feature_name]["avg"] * 100,
         NDCG_output_dict_mt[feature_name]["std"] * 100)
    ...
    latex_filling_uriel += "%s & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f \\\\\n" % \
        (symbol_dict[feature_name],
         NDCG_output_dict_mt[feature_name]["avg"] * 100,
         NDCG_output_dict_mt[feature_name]["std"] * 100,
         NDCG_output_dict_el[feature_name]["avg"] * 100,
    ...
    latex_filling_ranking += "%s & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f \\\\\n" % \
        ("LambdaRank-all",
         NDCG_output_dict_mt[feature_name]["avg"] * 100,
         NDCG_output_dict_mt[feature_name]["std"] * 100,
         NDCG_output_dict_el[feature_name]["avg"] * 100,
    ...
    latex_filling_ranking += "%s & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f \\\\\n" % \
        ("LambdaRank-dataset",
         ...
         NDCG_output_dict_parsing_dataset[feature_name]["avg"] * 100,
         NDCG_output_dict_parsing_dataset[feature_name]["std"] * 100)
    ...
    latex_filling_ranking += "%s & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f & %.1f $\\pm$ %.1f \\\\\n" % \
        ("LambdaRank-URIEL",
         ...
         NDCG_output_dict_parsing_uriel[feature_name]["avg"] * 100,
         NDCG_output_dict_parsing_uriel[feature_name]["std"] * 100)



    latex_tail = "\\midrule\n\\end{tabular}\n\\end{center}\n\\caption{\\label{tabel:MainResults} Our LambdaRank model leads to higher average NDCG@3 over the baselines on all three tasks: machine translation (MT), entity linking (EL), part-of-speech tagging (POS), and dependency parsing (Parsing). The reported baseline scores use the best-performing features per task.}\n\\end{table*}"
