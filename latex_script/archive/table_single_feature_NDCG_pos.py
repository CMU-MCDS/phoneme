...
def main():
    ...
    latex_tail = "\\midrule\n\\end{tabular}\n\\end{center}\n\\caption{\\label{tabel:SingleFeatureNDCGinPOS} The average and standard deviation of NDCG@3 over 26 task languages in POS tagging, using the LambdaRank model, each single statistical feature, and each single URIEL distance.}\n\\end{table}"
    ...
    with open(os.path.join(output_dir, "single_feature_pos.tex"), "w") as f:
