...
def main():
    ...
    for i in range(len(latex_filling_list)):
        latex_tail = "\\end{tabular}\n\\end{center}\n\\caption{\\label{tabel:SuggestedLangEL%d} Suggested transfer languages for entity linking compared to true best transfer languages. The numbers in the parentheses are the true ranking according to the accuracy. (Part %d)}\n\\end{table}" % (i, i)
        table_latex = latex_head + latex_filling_list[i] + latex_tail
        with open(os.path.join(output_dir, "suggested_lang_el_" + str(i) + ".tex"), "w") as f:
            print(table_latex, file=f)
    """

    latex_tail = "\\end{tabular}\n\\end{center}\n\\caption{\\label{tabel:SuggestedLangEL} Suggested transfer languages for entity linking compared to true best transfer languages. The numbers in the parentheses are the true ranking according to the accuracy.}\n\\end{table}"
    table_latex = latex_head + latex_filling_list[0] + latex_tail
    with open(os.path.join(output_dir, "suggested_lang_el.tex"), "w") as f:
        print(table_latex, file=f)
