...
def main():
    ...
    output_dir = os.path.join(root, "mt/output_mt_BLEUratio")
    with open(os.path.join(output_dir, "ratio.json"), "r") as f:
        NDCG_output_dict_mt = json.load(f)

    output_dir = os.path.join(root, "el/output_el_BLEUratio")
    with open(os.path.join(output_dir, "ratio.json"), "r") as f:
    ...
    output_dir = os.path.join(root, "mt/output_mt_BLEUratio_dataset")
    with open(os.path.join(output_dir, "ratio.json"), "r") as f:
        NDCG_output_dict_mt_dataset = json.load(f)

    output_dir = os.path.join(root, "el/output_el_BLEUratio_dataset")
    with open(os.path.join(output_dir, "ratio.json"), "r") as f:
    ...
    output_dir = os.path.join(root, "mt/output_mt_BLEUratio_uriel")
    with open(os.path.join(output_dir, "ratio.json"), "r") as f:
        NDCG_output_dict_mt_uriel = json.load(f)

    output_dir = os.path.join(root, "el/output_el_BLEUratio_uriel")
    with open(os.path.join(output_dir, "ratio.json"), "r") as f:
    ...
    latex_tail = "\\midrule\n\\end{tabular}\n\\end{center}\n\\caption{\\label{tabel:MainResultsRatio} On all three tasks: machine translation (MT), entity linking (EL), and part-of-speech tagging (POS), our LambdaRank model leads to higher average BLEU@3 (for MT) or accuracy@3 (for EL and POS) over the baselines. The reported baseline scores use the best-performing features per task.}\n\\end{table*}"
    ...
    with open(os.path.join(output_dir, "main_results_ratio.tex"), "w") as f:
