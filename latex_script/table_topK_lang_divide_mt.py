...
def main():
    ...
    for i in range(len(latex_filling_list)):
        latex_tail = "\\end{tabular}\n\\end{center}\n\\caption{\\label{tabel:SuggestedLangMT%d} Suggested transfer languages for machine translation compared to true best transfer languages. The numbers in the parentheses are the true ranking according to the BLEU scores. (Part %d)}\n\\end{table}" % (i+1, i+1)
        table_latex = latex_head + latex_filling_list[i] + latex_tail
        with open(os.path.join(output_dir, "suggested_lang_mt_%d.tex" % (i+1)), "w") as f:
