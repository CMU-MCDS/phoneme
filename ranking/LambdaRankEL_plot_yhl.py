...
if __name__ == "__main__":
    ...
    for task_lang_idx in range(task_lang_set.shape[0]):
        ...
        feature_names = ['o_w','s_tf','s_tk','s_tf/s_tk','d_gen','d_syn','d_fea','d_pho','d_inv','d_geo']
    ...
    for data_row_idx in range(1, data.shape[0]):
        ...
        if row[0] not in dict_task_transfer_genetic:
            dict_task_transfer_genetic[row[0]] = [(float(row[5+4]),float(row[2]))]
        else:
            dict_task_transfer_genetic[row[0]].append((float(row[5+4]),float(row[2])))
