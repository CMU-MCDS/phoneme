...
if __name__ == "__main__":
    ...
    data_file = "data_ranking_mt.csv" #Dataset-dependent features
    data = np.loadtxt(os.path.join(root, data_file), dtype=str, delimiter=",")
    
    uriel_file = 'uriel_distance.csv' #Uriel distance features
    uriel = np.loadtxt(os.path.join(root, uriel_file), dtype=str, delimiter=",")
    ...
    for i in range(lang_set.shape[0]):
        ...
        assert(data.shape[0] == uriel.shape[0])
        for i in range(1, data.shape[0]):
            ...
            data_depdt_features = row[5:]

            # Uriel features
            row2 = uriel[i]
            assert(str(row2[0]) == task_lang)
            assert(str(row2[1]) == aux_lang)
            uriel_features = row2[2:]


            features = np.append(data_depdt_features, uriel_features)
        ...
        print("================================")
        feature_name = np.append(data[0, 5:], uriel[0, 2:])
