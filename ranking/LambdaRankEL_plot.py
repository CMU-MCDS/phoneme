...
if __name__ == "__main__":
    ...
    data_file = "data_ranking_el.csv" #Dataset-dependent features
    ...
    single_feature_name_list = ["Entity overlap", "Transfer lang dataset size", "Target lang dataset size", "Transfer over target size ratio", "GENETIC", "SYNTACTIC", "FEATURAL", "PHONOLOGICAL", "INVENTORY", "GEOGRAPHIC"]
    ...
    for task_lang_idx in range(task_lang_set.shape[0]):
        test_lang_set = task_lang_set#[task_lang_set[task_lang_idx]]
        train_lang_set = task_lang_set#[lang for lang in aux_lang_set if lang not in test_lang_set]
        #assert(len(train_lang_set) == 53)
        ...
        for data_row_idx in range(1, data.shape[0]):
            ...
            # Features are:
            # ["Entity overlap", "Transfer lang dataset size", "Target lang dataset size", "Transfer over target size ratio", "GENETIC", "SYNTACTIC", "FEATURAL", "PHONOLOGICAL", "INVENTORY", "GEOGRAPHIC"]
            features = row[5:]
            ...
            if task_lang in test_lang_set and aux_lang in train_lang_set:
        ...
        #model.booster_.save_model(os.path.join(output_dir, "lgbm_model_el_" + task_lang_set[task_lang_idx] + ".txt"))
        # single_feature_name_list = ["Entity overlap", "Transfer lang dataset size", "Target lang dataset size", "Transfer over target size ratio", "GENETIC", "SYNTACTIC", "FEATURAL", "PHONOLOGICAL", "INVENTORY", "GEOGRAPHIC"]
        feature_names = ['o_w','s_tf','s_tg','s_tf/s_tg','d_gen','d_syn','d_fea','d_pho','d_inv','d_geo']
        ...
        print (len(predict_list_rank))
        num_to_plot = 53#min(10,len(predict_list_rank))
            ...
            for single_feature_idx in range(len(single_feature_name_list)):
                if sort_sign_list[single_feature_idx] != 0:
                    ...
                    for topK_k in range(PRINT_TOP_K):
                        ...
                        print (topK_k,aux_lang_got,true_rank_got)
    ...
    for data_row_idx in range(1, data.shape[0]):
        ...
        if row[0] not in dict_task_transfer_genetic:
            dict_task_transfer_genetic[row[0]] = [(float(row[9]),float(row[2]))]
        else:
            dict_task_transfer_genetic[row[0]].append((float(row[9]),float(row[2])))
