...
# Remove the parenthesis in the language code
...
if __name__ == "__main__":
    ...
    data_file = "data_ranking_pos.csv" #Dataset-dependent features
    ...
    for task_lang_idx in range(task_lang_set.shape[0]):
        test_lang_set = aux_lang_set#[task_lang_set[task_lang_idx]]
        train_lang_set = aux_lang_set#[lang for lang in aux_lang_set if lang not in test_lang_set]
        ...
        # single_feature_name_list = ["Overlap word-level", "Transfer lang dataset size", "Target lang dataset size", "Transfer over target size ratio", "Transfer lang TTR", "Target lang TTR", "Transfer target TTR distance", "GENETIC", "SYNTACTIC", "FEATURAL", "PHONOLOGICAL", "INVENTORY", "GEOGRAPHIC"]
        feature_names = ['o_w','s_tf','s_tg','s_tf/s_tg','ttr_tf','ttr_tg','d_ttr','d_gen','d_syn','d_fea','d_pho','d_inv','d_geo']
        ...
        num_to_plot = 59#min(10,len(predict_list_rank))
    ...
    plt.xticks(np.arange(1, 59+1, step=1))
    ...
    # plt.savefig("./figPOS.png")
    # plt.show()
    dict_task_transfer_genetic = {} # key = task, val = (genetic,acc)
    for data_row_idx in range(1, data.shape[0]):
        row = data[data_row_idx]
        if row[0] not in dict_task_transfer_genetic:
            dict_task_transfer_genetic[row[0]] = [(float(row[17]),float(row[2]))]
        else:
            dict_task_transfer_genetic[row[0]].append((float(row[17]),float(row[2])))
