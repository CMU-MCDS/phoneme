...
TRAIN_DIR = "./_transcribe"
if not os.path.exists(TRAIN_DIR):
  os.makedirs(TRAIN_DIR)

EXP_DIR = "./_exp_transcribe"
if not os.path.exists(EXP_DIR):
  os.makedirs(EXP_DIR)

DATA_DIR = "./_train/na_example_tiny/label"
...
def index():
  ...
  return redirect(url_for("transcribe", batch_size=batch_size))
...
@app.route("/transcribe/<batch_size>")
def transcribe(batch_size):
  # Input is casted into string, so restore their original type
  batch_size = int(batch_size)

  for folder in os.listdir(TRAIN_DIR):
    na_corpus = corpus.ReadyCorpus(os.path.join(TRAIN_DIR, folder), label_database_path=DATA_DIR, transcribe_new=True)
    na_reader = corpus_reader.CorpusReader(na_corpus, batch_size=batch_size, transcribe_new=True)
    model = rnn_ctc.Model(EXP_DIR, na_reader, num_layers=2, hidden_size=250, transcribe_new=True)
    model.transcribe(restore_model_path="./_exp/model/model_best.ckpt")
    # Expect only one training data folder...
    break
  return send_from_directory(os.path.join(EXP_DIR, "transcriptions"), "hyps.txt")
